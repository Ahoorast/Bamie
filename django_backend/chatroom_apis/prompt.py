...
do not respond with the exact response-template change the response-template and make it more user friendly"""
...
def singleRequestTemplate(id, input_example, output_example):
    template = ""
    template += f'<potential-request-template id={id}>'
    template += f'<request-template>'
    template += f'{input_example}'
    template += f'</request-template>'
    template += f'<response-template>'
    template += f'{output_example}'
    template += f'</response-template>'
    template += f'</potential-request-template>'
    return template

def OpenaiResponse(prompt, children):
    instructions = system_instruction
    for indx, child in enumerate(children):
        instructions += singleRequestTemplate(indx, child[0], child[1])
